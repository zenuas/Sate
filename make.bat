...
:release
	git archive HEAD --output=Sate-%DATE:/=%.zip
...
:test-all
	dotnet run --project test-all/Sate.TestAll.csproj --no-launch-profile -- %*
	@exit /b %ERRORLEVEL%

:bench
	dotnet run --project bench/Sate.Benchmark.csproj --no-launch-profile -c Release %*
	@exit /b %ERRORLEVEL%
